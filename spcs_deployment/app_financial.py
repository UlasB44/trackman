...
with tab2:
    ...
    if not bay_utilization.empty:
        ...
        with col2:
            st.markdown("#### Usage Distribution (Top 10 Bays)")
            top_bays = bay_utilization.head(10).copy()
            top_bays['BAY_LABEL'] = ['Bay ' + str(i+1) for i in range(len(top_bays))]
            fig = px.pie(
                top_bays,
        ...
        hourly_pattern = run_query("""
            ...
            FROM TRACKMAN_DW.GOLD.GLD_BAY_OCCUPATION_HEATMAP
            GROUP BY HOUR
            ORDER BY HOUR
        """)
